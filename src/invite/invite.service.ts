  ...
  async generateIcsForBooking(bookingId: string, requestingUserId?: string) {
    ...
    if (requestingUserId) {
      ...
      const student = booking.student_id
        ...
        : null;
      const tutorRow = booking.assigned_tutor_id
        ? await this.prisma.tutors.findUnique({
            where: { id: booking.assigned_tutor_id },
          })
        : null;
      const isParent = !!(
        student && student.parent_user_id === requestingUserId
      );
      const isAssignedTutor = !!(
        tutorRow && tutorRow.user_id === requestingUserId
      );
      if (!isParent && !isAssignedTutor) {
        // allow if admin? your system may have role-checking elsewhere
        throw new ForbiddenException(
          'Not allowed to fetch invite for this booking',
        );
      }
    }
    ...
    const studentUser =
      student && student.user_id
        ? await this.prisma.users.findUnique({ where: { id: student.user_id } })
        : null;

    // fetch tutor user
    let tutorUser: any = null;
    if (booking.assigned_tutor_id) {
      const tutorRow = await this.prisma.tutors.findUnique({
        where: { id: booking.assigned_tutor_id },
      });
      if (tutorRow)
        tutorUser = await this.prisma.users.findUnique({
          where: { id: tutorRow.user_id },
        });
    }
    ...
    const summary =
      `${subject?.name ?? 'Tutoring Session'} — ${pkg?.name ?? ''}`.trim();
